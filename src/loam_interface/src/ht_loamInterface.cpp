...
tf::TransformListener *transformListener_ptr=NULL;
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  ...
  try {
    ...
    // tf::poseTFToEigen(transformTf, transformationSensorToMap);
    tf::poseTFToEigen(transformTf, transformationSensorToMap);
  ...
  // Transform each point in the point cloud to the map frame
  for (int i = 0; i < laserCloud->size(); i++) {
    Eigen::Vector3d point(laserCloud->points[i].x, laserCloud->points[i].y, laserCloud->points[i].z);
    // Eigen::Vector3d point1(laserCloud->points[i].x, laserCloud->points[i].y, laserCloud->points[i].z);
    point = transformationSensorToMap.rotation()* point+transformationSensorToMap.translation() ;
    // point = 0*point;
    laserCloud->points[i].x = point.x();
    laserCloud->points[i].y = point.y();
    laserCloud->points[i].z = point.z();
  }
